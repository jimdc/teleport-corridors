...
- **Origin: built with OpenAI Codex CLI** (Feb 2026). Codex conventions are native
  here; any coding agent (Codex, Claude Code, opencode) may work on this repo.
...
- Work tracking: improvement work arrives as task cards under `backlog/` (YAML
  frontmatter with a runnable `verify:` gate, run from the repo root).
